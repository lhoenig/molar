...
- (void)tableView:(UITableView *)tableView
    ...
    if ([tableView cellForRowAtIndexPath:indexPath] == selectedCell) {
        ...
        CFPreferencesSetValue((CFStringRef)settingsKey, (CFStringRef) @"",
                              (CFStringRef)kBundleID, kCFPreferencesCurrentUser,
        ...
        CFPreferencesSetValue((CFStringRef)settingsKey,
    ...
    // CFPreferencesSetMultiple((CFDictionaryRef)settings, nil,
    // (CFStringRef)kBundleID, kCFPreferencesCurrentUser,
    // kCFPreferencesAnyHost);
    // CFPreferencesSynchronize((CFStringRef)kBundleID,
...
void handle_event(void *target, void *refcon, IOHIDServiceRef service,
    ...
    if (IOHIDEventGetType(event) == kIOHIDEventTypeKeyboard) {
        ...
        int usage =
            IOHIDEventGetIntegerValue(event, kIOHIDEventFieldKeyboardUsage);
        int down =
        ...
        if ((usage == CMD_KEY || usage == CMD_KEY_2) && !down)
            [[NSNotificationCenter defaultCenter]
                postNotificationName:@"SC_CmdKeyUp"
                              object:nil];
            ...
            [[NSNotificationCenter defaultCenter]
                postNotificationName:@"SC_CtrlKeyUp"
                              object:nil];
            ...
            [[NSNotificationCenter defaultCenter]
                postNotificationName:@"SC_AltKeyUp"
                              object:nil];
            ...
            [[NSNotificationCenter defaultCenter]
                postNotificationName:@"SC_KeyDown"
                              object:nil
                            userInfo:@{
                                @"key" : @(usage)
                            }];
...
- (id)initWithStyle:(UITableViewStyle)style {
    if ((self = [super initWithStyle:style])) {
        ...
        UIView *headerView = [[UIView alloc]
            initWithFrame:CGRectMake(0, 100, self.tableView.frame.size.width,
                                     130)];
...
- (void)saveShortcut {
    ...
    NSMutableArray *shortcuts = [NSMutableArray
        arrayWithArray:(NSArray *)CFBridgingRelease(
                           CFPreferencesCopyAppValue((CFStringRef)kShortcutsKey,
                                                     (CFStringRef)kBundleID))];
    NSMutableArray *shortcutNames = [NSMutableArray
        arrayWithArray:(NSArray *)CFBridgingRelease(CFPreferencesCopyAppValue(
                           (CFStringRef)kShortcutNamesKey,
                           (CFStringRef)kBundleID))];
    if (!shortcuts)
    ...
    NSString *shortcutKey = [@"shortcut-"
        stringByAppendingFormat:@"%@", [[NSUUID UUID] UUIDString]];
    NSString *eventName =
        [kBundleID stringByAppendingFormat:@".%@", shortcutKey];
    ...
    CFPreferencesSetValue((CFStringRef)kShortcutsKey, (CFArrayRef)shortcuts,
                          (CFStringRef)kBundleID, kCFPreferencesCurrentUser,
                          kCFPreferencesAnyHost);
    CFPreferencesSetValue((CFStringRef)kShortcutNamesKey,
                          (CFArrayRef)shortcutNames, (CFStringRef)kBundleID,
                          kCFPreferencesCurrentUser, kCFPreferencesAnyHost);
    // CFPreferencesSynchronize((CFStringRef)kBundleID,
    // kCFPreferencesCurrentUser, kCFPreferencesAnyHost);
    CFPreferencesAppSynchronize((CFStringRef)kBundleID);
...
+ (NSArray *)characters {
    ...
    NSString *activeLayout =
        (NSString *)CFBridgingRelease(CFPreferencesCopyAppValue(
            (CFStringRef)kKeyboardLayoutKey, (CFStringRef)kBundleID));
...
- (void)keyDown:(NSNotification *)notification {
    int key =
        ((NSNumber *)[notification.userInfo objectForKey:@"key"]).intValue;
...
- (id)initWithStyle:(UITableViewStyle)style {
    if ((self = [super initWithStyle:style])) {
        ...
        CFPreferencesAppSynchronize((CFStringRef)kBundleID);
        shortcuts =
            [NSMutableArray arrayWithArray:(NSArray *)CFBridgingRelease(
                                               CFPreferencesCopyAppValue(
                                                   (CFStringRef)kShortcutsKey,
                                                   (CFStringRef)kBundleID))];

        [[NSNotificationCenter defaultCenter]
            addObserver:self
               selector:@selector(reloadTable)
                   name:@"de.hoenig.molar-preferencesChanged-nc"
                 object:nil];
        [[NSNotificationCenter defaultCenter]
            addObserver:self
               selector:@selector(reloadTable)
...
- (void)tableView:(UITableView *)tableView
    ...
    if ([[self tableView:tableView cellForRowAtIndexPath:indexPath]
        ...
        LAEventSettingsController *vc = [[LAEventSettingsController alloc]
            initWithModes:[NSArray arrayWithObjects:@"springboard",
                                                    @"application", nil]
...
- (id)getValueForSpecifier:(PSSpecifier *)specifier {
    ...
    NSMutableString *plistPath = [[NSMutableString alloc]
        initWithString:[specifierProperties
                           objectForKey:kPrefs_KeyName_Defaults]];
    if (plistPath) {
        NSDictionary *dict =
            (NSDictionary *)[self initDictionaryWithFile:&plistPath
                                               asMutable:NO];
...
- (void)tableView:(UITableView *)tableView
    ...
    if (indexPath.section == 4 || indexPath.section == 5) {
        ...
        asc.settingsKey =
            [[[[self specifiersInGroup:7] objectAtIndex:(int)indexPath.row + 1]
...
- (id)initDictionaryWithFile:(NSMutableString **)plistPath
                   asMutable:(BOOL)asMutable {
    if ([*plistPath hasPrefix:@"/"])
        *plistPath = [NSMutableString
            stringWithString:[NSString
                                 stringWithFormat:@"%@.plist", *plistPath]];
    else
        *plistPath = [NSMutableString
            stringWithString:[NSString stringWithFormat:@"%@/%@.plist",
                                                        kPrefs_Path,
                                                        *plistPath]];
...
- (id)init {
    if ((self = [super init])) {
        ...
        CFNotificationCenterAddObserver(
            ...
            CFSTR("de.hoenig.molar-preferencesChanged"), NULL,
